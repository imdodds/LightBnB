...
const getAllProperties = (options, limit = 10) => {
  ...
  if (options.city) {
    ...
    if (options.owner_id || options.minimum_price_per_night || options.maximum_price_per_night || options.minimum_rating) {
      queryString += ` AND `;
    }
  }
  ...
  if (options.owner_id) {
    ...
    if (!options.city) queryString += `WHERE `;
  ...
  if (options.minimum_price_per_night) {
    ...
    if (!options.city && !options.owner_id) queryString += `WHERE `;
